...
/*
  ...
  此时直到这里，如果用户不断的调用fn，这时延迟处理函数一次都不会执行。
  故需要一个时间段，在这个时间段里至少触发一次。

  当用户触发fn的时候应该在某段时间内至少触发一次，既然是在某段时间内，
  那么这个判断条件就可以取当前的时间毫秒数，每次函数调用把当前的时间和上一次调用时间相减，
  然后判断差值如果大于某段时间就直接触发，否则还是走timeout的延迟逻辑。

  注意！setTimeout 函数返回值应该保存在一个相对全局变量里面，
  否则每次 resize 的时候都会产生一个新的计时器，这样就达不到我们发的效果了。

  为了达到上面的要求，我们用一个闭包函数（throttle节流）把 timer 放在内部并且返回延时处理函数，
  这样以来 timer 变量对外是不可见的，但是内部延时函数触发时还可以访问到 timer 变量。
*/
...
function throttle(fn, wait = 2000, atLeast = 0) {
  ...
  return function (...args) {
    let currentTime = Date.now() //记录当前的事件点
